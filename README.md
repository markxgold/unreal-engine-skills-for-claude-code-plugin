...
> **Note:** This plugin does not ship a static `.mcp.json` file. The `.mcp.json` is dynamically generated by the ModelContextProtocol Unreal plugin on editor startup (via `ModelContextProtocol.GenerateClientConfig`), so the MCP server configuration always reflects the actual runtime port and endpoint.
